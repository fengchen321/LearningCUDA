...
# NVIDIA Nsight Systems (V13.0.88)
...
MPS Options:
  --mps arg (=none)                     Select MPS behavior:
                                          none
                                          (No MPS support)
                                          client
                                          (Launch MPS client process)
                                          primary-client
                                          (Launch primary MPS client process)
                                          control
                                          (Launch MPS ncu control)
  --mps-num-clients arg                 Number of MPS client processes.
  --mps-timeout arg                     Timeout (seconds) to discover MPS client processes.
...
Launch Options:
  --check-exit-code arg (=1)            Check the application exit code and print an error if it is different than 0. 
                                        ...
                                        code is not 0.
  --forward-signals                     Forwards all the signals to the application root process.
  ...
  --call-stack-type arg                 Set the call stack type to be collected. More than one type may be specified. 
                                        Implies --call-stack.
                                          ...
                                          python
                                          (collect a Python CPU call stack)
  ...
  --nvtx-push-pop-scope arg (=thread)   Specify scope of push-pop ranges:
                                           thread (default) : NVTX Push/Pop ranges are scoped per thread.
...
Common Profile Options:
  ...
  --app-replay-mode arg (=balanced)     Application replay kernel matching mode:
                                          strict   : Requires all filtered kernels to match across all
                                                     replay passes in the exact order.
                                          balanced : Requires all filtered kernels to match across all
                                          (default)  replay passes, without strict ordering.
                                          relaxed  : Produces results only for filtered kernels that could
                                                     be matched across replay passes. Unmatched kernels are
                                                     dropped.
  ...
  --range-replay-options arg            Range replay options, separated by comma:
                                          ...
                                          disable-host-save
                                          (Disable saving device-mapped host allocations for every kernel but capture 
                                        it only once before the first kernel launch)
                                          disable-dependent-kernel-detection
                                          (Disable dependent kernel detection before capturing device-mapped host 
  ...
  --import-sass arg (=1)                Import ELF cubins into the report. Cubins include SASS, PTX and meta 
  ...
  --query-metrics-collection arg (=profiling)
                                        ...
                                        Available modes:
                                          device
                                          (query CUDA device attributes)
                                          groups
                                          (query metric groups)
                                          launch
                                          (query launch attributes)
                                          numa
                                          (query NUMA topology metrics)
                                          nvlink
                                          (query metrics related to NVLink)
                                          pmsampling
                                          ...
                                          stats
                                          (query metrics generated by the profiler to inform about profiling 
                                        statistics)
                                          warpsampling
                                          (query metrics available for periodic sampling of the warp program counter 
                                        and warp scheduler state)
  ...
  --pipeline-boost-state arg (=stable)  Control the Tensor Core boost state. Setting stable Tensor Core boosting is 
                                        recommended for application performance profiling ensuring predictive run to 
                                        run performance.
                                          stable
                                          (Set the Tensor Core boost state to stable)
                                          dynamic
                                          (Set the Tensor Core boost state to dynamic)
...
Filter Profile Options:
  ...
  --range-filter arg                    Filter to profile specified instance(s) of matching NVTX ranges or start/stop 
                                        ...
                                        --nvtx-include.
  --native-include arg                  Adds include statement to the native CPU call stack filter, which allows 
                                        selecting kernels to profile based on native CPU stack frames.
  --native-exclude arg                  Adds exclude statement to the native CPU call stack filter, which allows 
                                        selecting kernels to profile based on native CPU stack frames.
  --python-include arg                  Adds include statement to the python CPU call stack filter, which allows 
                                        selecting kernels to profile based on python CPU stack frames.
  --python-exclude arg                  Adds exclude statement to the python CPU call stack filter, which allows 
                                        selecting kernels to profile based on python CPU stack frames.
...
Console Output Options:
  ...
  --print-rule-details                  Print additional details for rule results, such as tables and metrics that are 
                                        key performance indicators. Currently has no effect in CSV mode.
...
Usage of --native-include or --python-include and --native-exclude or --python-exclude:
  ncu --native-include "Module A@File A@Function A" --native-exclude "Module B@File B@Function A"
    Profile kernels launched via Function A of File A and Module A but not File B and Module B in the CPU call stack.
  ncu --python-include "Module A@File A@Function A" --python-exclude "Module B@File B@Function A"
    Profile kernels launched via Function A of File A and Module A but not File B and Module B in the Python call stack.
  ncu --native-include "Module A@@[Function A"
    Profile kernels launched via Function A of Module A present at the bottom of the CPU call stack.
  ncu --python-exclude "File A@Function A]"
    Profile all kernels except kernels launched via Function A of File A present at the top of the Python call stack.
  Note: Module and File names are optional. If not provided, the tool will match all modules and files. For both module and file, you can specify any path to them or simply their names.
